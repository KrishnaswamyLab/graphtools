...
def _numba_build_kernel_to_data_optimized(pdx, bandwidth, decay, thresh):
    ...
    bandwidth_is_scalar = bandwidth.ndim == 0 or (
        bandwidth.ndim == 1 and bandwidth.size == 1
    )
    ...
    def build_kernel_to_data(
        ...
        if self.decay is None or self.thresh == 1:
            ...
            with _logger.log_task("affinities"):
                if bandwidth is None:
                    if NUMBA_AVAILABLE:
                        bandwidth = _numba_compute_bandwidths(
                            distances, knn, bandwidth_scale
                        )
                    else:
                ...
                if NUMBA_AVAILABLE:
                    ...
                    radius = bandwidth * np.power(
                        -1 * np.log(self.thresh), 1 / self.decay
                    )
                    update_idx = np.argwhere(
                        np.max(distances, axis=1) < radius
                    ).reshape(-1)
                _logger.log_debug(
...
class LandmarkGraph(DataGraph):
    ...
    def set_params(self, **params):
        ...
        if "n_svd" in params and params["n_svd"] != self.n_svd:
            ...
            reset_landmarks = True
        if (
            "random_landmarking" in params
            and params["random_landmarking"] != self.random_landmarking
        ):
