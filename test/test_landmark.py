...
def test_landmark_with_non_euclidean_distances():
    ...
    different_from_euclidean = (
        not np.array_equal(euclidean_clusters, manhattan_clusters) or
        not np.array_equal(euclidean_clusters, cosine_clusters)
    )
    ...
    print(f"Landmark operator sums: euclidean={euclidean_landmark_sum:.6f}, "
    ...
    operators_different = (
        abs(euclidean_landmark_sum - manhattan_landmark_sum) > 1e-10 or
        abs(euclidean_landmark_sum - cosine_landmark_sum) > 1e-10
    )
